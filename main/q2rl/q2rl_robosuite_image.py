...
flags.DEFINE_string("demo_replay_path", None, "path to load the demo replay buffer from")
...
def main(_):
    ...
    def evaluate_and_log_results(
        ...
        print_green(f"Evaluation at step {step_number}: {eval_info}")
    ...
    for _ in tqdm.tqdm(range(step, FLAGS.num_online_steps + FLAGS.num_q_est_steps)):
        ...
        if step < FLAGS.num_q_est_steps:
            ...
            action, sample_info = agent.sample_actions(observation, bc_action=bc_action, bc_log_prob=bc_log_prob, bc_entropy=bc_entropy, argmax=True, log=True)
