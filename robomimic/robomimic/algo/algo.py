...
class RolloutPolicy(object):
    ...
    def __call__(self, ob, goal=None, log_prob=False):
        """
        ...
        if log_prob:
            ac, logp, entropy = self.policy.get_action_and_log_prob(obs_dict=ob, goal_dict=goal)
            return TensorUtils.to_numpy(ac[0]), TensorUtils.to_numpy(logp[0]), TensorUtils.to_numpy(entropy[0])
        else:   
