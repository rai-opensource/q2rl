...
from backend.common.typing import Params, PRNGKey
...
class ModuleDict(nn.Module):
    ...
    def __call__(self, *args, name=None, **kwargs):
        """
        Call one or all modules in the dict.

        If name is None, initializes all modules using kwargs keyed by module name.
        If name is provided, calls only that module with the given args/kwargs.

        Args:
            *args: Positional arguments forwarded to the named module.
            name: Name of the module to call. If None, initializes all modules.
            **kwargs: When name is None, maps module names to their arguments.
                      Otherwise, forwarded to the named module.

        Returns:
            Output of the named module, or dict of outputs from all modules.
        """
        if name is None:
...
class JaxRLTrainState(struct.PyTreeNode):
    ...
    def _tx_tree_map(*args, **kwargs):
        """
        jax.tree_map that treats optax.GradientTransformation leaves as atomic nodes.

        This prevents tree_map from recursing into optimizer internals when mapping
        over the txs pytree.
        """
        return jax.tree_map(
            *args,
