...
class SACAgent(flax.struct.PyTreeNode):
    ...
    def forward_policy_and_sample(
        ...
        *,
        ...
        """
        Forward pass for policy and sample actions from it.

        Args:
            obs: Observations to condition the policy on.
            rng: Random number generator key.
            grad_params: Optional non-default parameters for gradient computation.
            repeat: If set, sample this many actions per observation.

        Returns:
            Tuple of (actions, log_probs). If repeat is set, shapes are
            (batch, repeat, action_dim) and (batch, repeat) respectively.
        """
    ...
    def policy_loss_fn(self, batch, params: Params, rng: PRNGKey):
        ...
        Maximizes Q-values minus entropy regularization. Optionally includes a
        BC regularization term weighted by bc_loss_weight.

        Args:
            batch: Batch dict with 'observations' and 'actions'.
    ...
    def temperature_loss_fn(self, batch, params: Params, rng: PRNGKey):
        """
        Compute the SAC temperature (entropy coefficient) loss.

        Adjusts temperature to maintain a target entropy via the Lagrange
        penalty mechanism.

        Args:
            batch: Batch dict (only next_observations are used).
            params: Parameters to differentiate with respect to.
            rng: Random number generator key.

        Returns:
            Tuple of (temperature_loss, info_dict).
        """
        rng, next_action_sample_key = jax.random.split(rng)
        next_actions, next_actions_log_probs = self._compute_next_actions(
            batch, next_action_sample_key
    ...
    def loss_fns(self, batch):
        """
        Return a dict of loss functions for each network component.

        Returns:
            Dict mapping network names ('critic', 'actor', 'temperature') to
            partially applied loss functions that take (params, rng) as arguments.
        """
        return {
    ...
    def get_debug_metrics(self, batch, **kwargs):
        """
        Compute debug metrics for a batch without updating parameters.

        Args:
            batch: Batch dict with observations, actions, rewards, etc.

        Returns:
            Dict of merged critic and actor loss info metrics.
        """
