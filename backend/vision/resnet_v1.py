"""
ResNet-based vision encoders for RL.

Provides ResNet architectures and utility modules for processing image observations
in reinforcement learning. Includes spatial coordinate adding and spatial softmax.
"""
...
from backend.vision.film_conditioning_layer import FilmConditioning
...
class AddSpatialCoordinates(nn.Module):
    """
    Add spatial coordinate channels to image features.
    
    Appends normalized 2D coordinate grids to the last dimension of input.
    Coordinates are normalized to [-1, 1] range.
    
    Attributes:
        dtype: Data type for coordinates (default float32).
    """
...
class SpatialSoftmax(nn.Module):
    """
    Spatial soft-argmax operation on feature maps.
    
    Converts spatial feature maps to spatial keypoint locations using sofmax
    attention. Produces a vector of (x, y) coordinates weighted by feature
    activation.
    ...
    Attributes:
        ...
        pos_x: Pre-computed x position grid.
        pos_y: Pre-computed y position grid.
        temperature: Softmax temperature (-1 for learnable, otherwise fixed).
        log_heatmap: Whether to return log heatmap (default False).
    """
    height: int
    width: int
    channel: int
